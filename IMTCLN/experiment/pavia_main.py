...
def build_class_reps_and_covariance_estimates(context_features, context_labels):
    ...
    for c in torch.unique(context_labels):
        ...
        class_precision_matrices[c.item()] = torch.inverse(
            ...
            # + torch.eye(class_features.size(1), class_features.size(1)).cuda(0))+torch.ones(class_features.size(1), class_features.size(1)).cuda(0)*1e-6
...
for jjj in range(10):
    ...
    X = sio.loadmat('/home/sxp/sxp-2/data/pavia.mat')
    X = np.array(X['pavia'])
    label = sio.loadmat('/home/sxp/sxp-2/data/pavia_label.mat')
    label = np.array(label['pavia_label']).reshape([207400,1])
    label_short = label[np.where(label != 0)[0], :]
    label_short2 = label[np.where(np.logical_and(label != 0, label != 9))[0], :]
    ...
    ww = torch.from_numpy(np.load('ww2.npy')).to(device)
    nei_w = torch.from_numpy(np.load('nei_w2.npy')).to(device)
    nei_b = torch.from_numpy(np.load('nei_b2.npy')).to(device)
    ...
    cost_D1, loss_cross_entropy1, f_loss1, f_loss21, loss_mse41, loss1, un_acc = [], [], [], [], [], [], []
    acc3 = []
    D_Loss = list()
    C_Loss = list()
    Pd_Acc = list()
        ...
        for j in range(iterations):
            with torch.set_grad_enabled(True):
                ...
                for class_label in range(num_class):
                    indices_in_class = torch.nonzero(label_tensor_indices2 == class_label).squeeze()
                ...
                specified_labels = label_tensor_indices2[specified_indices]
                # 获取剩余标签对应的特征向量
                remaining_indices = torch.tensor([i for i in range(len(label_tensor_indices2)) if i not in specified_indices]).to(device)
                remaining_features = features[remaining_indices]
                remaining_labels = label_tensor_indices2[remaining_indices]
                ...
                loss_mse4 = torch.tensor(0.0).to(device)
                if i == 100:
